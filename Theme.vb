...
#Region "Properties"
    ...
    Protected Overrides Sub OnSizeChanged(e As EventArgs)
        ...
        If _ShowText AndAlso (Height < Font.Size * 2 + 3) Then
            Size = New Size(Width, CInt(Font.Size * 2 + 3))
        End If
        Invalidate()
    End Sub
...
#Region "Properties"
    ...
    Private Function getPointedTab() As TabPage
        For i As Integer = 0 To TabPages.Count - 1
            If GetTabRect(i).Contains(PointToClient(Cursor.Position)) Then
                Return TabPages(i)
            End If
        Next
    ...
    Private Sub swapTabPages(src As TabPage, dst As TabPage)
        Dim srci As Integer = TabPages.IndexOf(src)
        Dim dsti As Integer = TabPages.IndexOf(dst)

        TabPages(dsti) = src
        TabPages(srci) = dst

        If SelectedIndex = srci Then
            SelectedIndex = dsti
        ElseIf SelectedIndex = dsti Then
            SelectedIndex = srci
        End If
